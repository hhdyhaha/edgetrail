...
function SitesPage() {
  ...
  return (
    <AppShell
      ...
      active="Sites"
      subtitle="Create and monitor allowed analytics sites."
      title="Sites"
    >
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0 flex-1 rounded-lg border border-[#dee3ea] bg-white px-5 py-4 shadow-[0_5px_0_rgba(15,23,42,0.05)] dark:border-slate-800 dark:bg-slate-950">
          ...
          <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Allowed analytics sites and their current status.
          </div>
        </div>
        <Badge>{sites.length} active sites</Badge>
      </div>
        ...
        {sites.length === 0 ? (
          <div className="p-8">
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              Create a site to generate a tracking script and begin collecting events.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] text-left text-sm">
              <thead className="border-b border-[#dee3ea] text-xs font-semibold text-slate-500 dark:border-slate-800 dark:text-slate-400">
                <tr>
                  <th className="px-7 py-5">Site</th>
                  <th className="px-7 py-5">Domain</th>
                  <th className="px-7 py-5">Status</th>
                  <th className="px-7 py-5">Share</th>
                </tr>
              </thead>
              <tbody>
                {sites.map((site) => (
                  ...
                  >
                    <td className="px-7 py-5">
                      ...
                      <div className="mt-1 text-xs text-slate-500">public id {site.id}</div>
                    </td>
                    ...
                    <td className="px-7 py-5">
                      <Badge tone="slate">Configured in settings</Badge>
                    </td>
                  </tr>
                ))}
              </tbody>
